...
from benchmark_mps.utils.backend import current_backend, to_numpy
...
def nullspace(matrix: np.ndarray, tol: float = 1e-10) -> np.ndarray:
    ...
    rank = int(to_numpy(xp.sum(s > tol)))
...
def pretty_mat(mat: np.ndarray, tol: float = 1e-12, digits: int = 6) -> str:
    ...
    xp = backend.xp
    out = mat.copy()
    out[xp.abs(out) < tol] = 0
    return np.array2string(to_numpy(out), precision=digits, suppress_small=True)
